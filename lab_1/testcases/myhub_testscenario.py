...
def test_hub():
    ...
    test = new_packet("20:00:00:00:00:01", "ff:ff:ff:ff:ff:ff", "192.168.1.100",'172.16.42.2')
    s.expect(PacketInputEvent("eth1", test, display=Ethernet),"An Ethernet frame with a broadcast destination address should arrive on eth1")
    s.expect(PacketOutputEvent("eth0", test, "eth2", test, display=Ethernet),"The Ethernet frame with a broadcast destination address should be forwarded out ports eth0 and eth2")
